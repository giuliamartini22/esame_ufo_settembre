...
class Model:
    ...
    def buildGraph(self, lat, long, shape):
        ...
        self._listEdges = DAO.getAllEdges(lat, long)
        print(self._listEdges)
        self._listaPesi = DAO.getPesi(lat, long, shape)
        print(self._listaPesi)

        for e in self._listEdges:
            s1 = self._idMap[e[0]]
            s2 = self._idMap[e[1]]
            peso = 0
            for p in self._listaPesi:
                if s1.id == p[0] or s2.id == p[0]:
                    peso = peso + p[1]
                    print(s1, s2, peso)
